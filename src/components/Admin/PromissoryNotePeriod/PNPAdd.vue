  ...
  <div>
    ...
    <div class="q-pa-md">
      <q-card bordered class="my-card" elevated :square="true">
        <q-card-section>
          <!-- <div><q-icon name="label_important" class="text-h6 text-red-15" /> Promissory Note Period Information</div> -->
  ...
  export default {
    ...
    methods: {
      ...
      async getSpecific(){
        ...
        let { data, status } = await this.$store.dispatch("promissory_note_period/getSpecific", payload);
        console.log(data);
      ...
      async submit() {
        ...
        if (await this.validate()) {
          let payload = {
            promi_days: vm.promi_days
          }

          let endpoint = "promissory_note_period/add";
          ...
          if(vm.is_update){
            ...
            endpoint = "promissory_note_period/update";
