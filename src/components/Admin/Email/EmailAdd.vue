  ...
  <div>
    <div class="q-pa-md">
      <q-card bordered class="my-card" elevated :square="true">
        <q-card-section>
          <div><q-icon name="label_important" class="text-h6 text-red-15" /> Email Information</div>
...
<script>
  ...
  import RichText from "components/AddOns/RichText.vue";
  ...
  export default {
    ...
    methods: {
      ...
      async getSpecific(){
        ...
        let { data, status } = await this.$store.dispatch("email/getSpecificEmail", payload);
        console.log(data);
      ...
      async submit() {
        ...
        if (await this.validate()) {
          let payload = {
            code: vm.code.toUpperCase(),
            desc: vm.desc.toUpperCase(),
            subject: vm.subject.toUpperCase(),
            body: vm.body.toUpperCase()
          }

          let endpoint = "email/addEmail";
          ...
          if ([200, 201].includes(status)) {
            ...
            Notify.create({
              message: `Successfully ${success_message} Email Template.`,
              position: 'top-right',
              closeBtn: "X",
              timeout: 2000,
              color: 'green',
            })
            this.$router.push({name: 'email-lists'})
          } else {
