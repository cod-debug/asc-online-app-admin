...
<script>
  export default {
    ...
    data: () => ({
      columns: [
        { name: 'code', align: 'left', label: 'Code', field: 'code', sortable: false },
    ...
    methods: {
      ...
      async getAppType() {
        ...
        let { data, status } = await vm.$store.dispatch("email/getAllEmail", payload);
      ...
      viewDetails (evt, row) {
        this.$router.push({name: 'email-update', params: {
          id: row.id
        }});
      },
      closeModal() {
