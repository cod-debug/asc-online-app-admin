  ...
  <div>
    <div class="q-pa-md">
      <q-card bordered class="my-card" elevated :square="true">
        <q-card-section>
          <div><q-icon name="label_important" class="text-h6 text-red-15" /> Type of Rationale Information</div>
...
<script>
  ...
  export default {
    data: () => ({
      ...
      category_choices: ["FOR COMPLIANCE", "FOR INCOMPLETE DECISION", "FOR TOA APPLICATION"]
    ...
    methods: {
      ...
      async getSpecific(){
        ...
        let { data, status } = await this.$store.dispatch("rationale/getSpecific", payload);
        console.log(data);
      ...
      async submit() {
        ...
        if (await this.validate()) {
          let payload = {
            rationale: vm.rationale.toUpperCase(),
            category: vm.category.toUpperCase()
          }

          let endpoint = "rationale/add";
          ...
          if ([200, 201].includes(status)) {
            ...
            Notify.create({
              message: `Successfully ${success_message} Rationale Type.`,
              position: 'top-right',
              closeBtn: "X",
              timeout: 2000,
              color: 'green',
            })
            this.$router.push({name: 'rational-lists'})
          } else {
